...
def copy_logs_to_redis(conn, path, channel, count=10,
    ...
    bytes_in_redis = 0
    waiting = deque()
    create_chat(conn, 'source', list(
        map(str, list(range(count)))), '', channel)  # I
...
class TestCh06(unittest.TestCase):
    ...
    def test_distributed_locking(self):
        ...
        self.assertFalse(acquire_lock_with_timeout(
            self.conn, 'testlock', .01, 1))
    ...
    def test_delayed_tasks(self):
        ...
        for delay in [0, .5, 0, 1.5]:
            self.assertTrue(execute_later(
                self.conn, 'tqueue', 'testfn', [], delay))
    ...
    def test_multi_recipient_messaging(self):
        self.conn.delete('ids:chat:', 'msgs:1', 'ids:1',
                         'seen:joe', 'seen:jeff', 'seen:jenny')
    ...
    def test_file_distribution(self):
        ...
        import threading
        import binascii
        self.conn.delete('test:temp-1.txt', 'test:temp-2.txt', 'test:temp-3.txt',
                         'msgs:test:', 'seen:0', 'seen:source', 'ids:test:', 'chat:test:')
        ...
        try:
            ...
            def callback(conn, line):
                if line is None:
                    print("Finished with a file %s, linecount: %s" %
                          (index[0], counts[index[0]]))
                    index[0] += 1
